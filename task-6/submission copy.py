...
"""
def get_disparity2(im1, im2, max_disp, win_size):
...
def get_disparity(im1, im2, max_disp, win_size):
    ...
    ones_mask = np.ones((win_size, win_size))  

    for d in range(max_disp + 1):
        shifted_im2 = np.roll(im2, -d, axis=1) 
        squared_diff = (im1 - shifted_im2) ** 2  

        cost = convolve2d(squared_diff, ones_mask, mode='same', boundary='fill', fillvalue=0)

        if d == 0:
            min_cost = cost
            dispM.fill(0) 
        else:
            update_mask = cost < min_cost  
            min_cost[update_mask] = cost[update_mask]
            dispM[update_mask] = d  
